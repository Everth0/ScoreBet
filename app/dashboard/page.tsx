    ...
    <main style={{background:'#0A0E1A', minHeight:'100vh', color:'#F9FAFB', fontFamily:'Inter, sans-serif'}}>
      ...
      <style>{`
        ...
        * { box-sizing:border-box; }
        ...
        @media(max-width:900px) { .dashboard-grid { grid-template-columns:1fr !important; } .betslip-desktop { display:none !important; } .mobile-bet-btn { display:flex !important; } }
      `}</style>
      ...
      <div style={{paddingTop:'64px'}}>
          ...
          <div style={{display:'flex', flexDirection:'column', gap:'20px'}}>
            ...
            <div style={{background:'#111827', border:'1px solid rgba(0,255,136,0.15)', borderRadius:'14px', padding:'22px'}}>
              ...
              <button onClick={verAnuncio} disabled={adLoading || adsHoy >= 10} className="ad-btn"
                ...
                {adLoading ? '⏳ Espera 5 segundos...' : adsHoy >= 10 ? '✅ Limite diario alcanzado' : '📺 Ver anuncio — +50 pts'}
              </button>
              {adMsg && (
              ...
              <Link href="/mis-apuestas" style={{textDecoration:'none'}}>
                <div style={{background:'#111827', border:'1px solid rgba(59,130,246,0.2)', borderRadius:'12px', padding:'16px', display:'flex', alignItems:'center', gap:'12px', cursor:'pointer'}}>
                  ...
                  <div>
                    ...
                    <div style={{fontSize:'11px', color:'#6B7280'}}>Ver historial</div>
                  </div>
                </div>
              </Link>
              <Link href="/apuestas-reales" style={{textDecoration:'none'}}>
                <div style={{background:'#111827', border:'1px solid rgba(245,158,11,0.2)', borderRadius:'12px', padding:'16px', display:'flex', alignItems:'center', gap:'12px', cursor:'pointer'}}>
                  ...
                  <div>
                    ...
                    <div style={{fontSize:'11px', color:'#6B7280'}}>Bono hasta $200</div>
                  </div>
                </div>
              </Link>
            </div>
            ...
            <div>
              <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'14px'}}>
                <h3 style={{fontFamily:'Rajdhani, sans-serif', fontSize:'18px', fontWeight:700}}>Partidos destacados</h3>
                <Link href="/partidos" style={{fontSize:'13px', color:'#00FF88', textDecoration:'none', fontWeight:500}}>Ver todos →</Link>
              </div>

              {betMsg && (
                <div style={{marginBottom:'12px', padding:'12px', borderRadius:'8px', background: betMsg.startsWith('✅') ? 'rgba(0,255,136,0.08)' : 'rgba(239,68,68,0.08)', border:`1px solid ${betMsg.startsWith('✅') ? 'rgba(0,255,136,0.2)' : 'rgba(239,68,68,0.2)'}`, fontSize:'13px', color: betMsg.startsWith('✅') ? '#00FF88' : '#EF4444'}}>
                  {betMsg}
                </div>
              )}
              ...
              <div style={{display:'flex', flexDirection:'column', gap:'10px'}}>
                {matches.map(m => (
                  <div key={m.id} style={{background:'#111827', border:`1px solid ${selectedBet?.matchId === m.id ? 'rgba(0,255,136,0.3)' : 'rgba(255,255,255,0.06)'}`, borderRadius:'14px', padding:'16px'}}>
                    ...
                    <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'12px'}}>
                      <div style={{display:'flex', alignItems:'center', gap:'8px'}}>
                        <span style={{fontSize:'22px'}}>{m.homeIcon}</span>
                      ...
                      <div style={{display:'flex', alignItems:'center', gap:'8px'}}>
                        ...
                        <span style={{fontSize:'22px'}}>{m.awayIcon}</span>
                      </div>
                    </div>
        ...
        {selectedBet && (
          <button onClick={() => setShowBetslip(true)} className="mobile-bet-btn"
