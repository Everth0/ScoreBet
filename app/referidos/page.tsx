      ...
      <div style={{paddingTop:'64px'}}>
        ...
        <div style={{background:'#0F1520', borderBottom:'1px solid rgba(255,255,255,0.06)', padding:'36px 24px'}}>
          ...
          <p style={{fontSize:'15px', color:'#9CA3AF', maxWidth:'500px'}}>
            Por cada amigo que use tu codigo, tu y el reciben <span style={{color:'#00FF88', fontWeight:600}}>+300 puntos</span> automaticamente.
          </p>
        </div>
        ...
        <div style={{padding:'32px 24px', display:'flex', flexDirection:'column', gap:'24px', maxWidth:'800px', margin:'0 auto', animation:'fadeIn .4s ease'}}>
          ...
          {codigo ? (
            <div style={{background:'#111827', border:'1px solid rgba(0,255,136,0.2)', borderRadius:'16px', padding:'28px', textAlign:'center'}}>
              ...
              <div style={{fontFamily:'JetBrains Mono,monospace', fontSize:'clamp(28px,6vw,48px)', fontWeight:700, color:'#00FF88', letterSpacing:'8px', marginBottom:'8px', background:'rgba(0,255,136,0.05)', border:'1px solid rgba(0,255,136,0.15)', borderRadius:'12px', padding:'16px'}}>
                {codigo}
              </div>
              <div style={{fontSize:'12px', color:'#6B7280', marginBottom:'20px', fontFamily:'JetBrains Mono,monospace'}}>
                {linkReferido}
              </div>
              <div style={{display:'flex', gap:'12px', justifyContent:'center', flexWrap:'wrap'}}>
                <button onClick={() => copiar(codigo)} className="copy-btn"
                  style={{padding:'12px 24px', borderRadius:'10px', background:'#00FF88', color:'#0A0E1A', fontWeight:700, fontSize:'14px', border:'none', cursor:'pointer', transition:'all .2s', fontFamily:'Inter,sans-serif'}}>
                  {copiado ? '✅ Copiado!' : '📋 Copiar codigo'}
                </button>
                <button onClick={() => copiar(linkReferido)} className="share-btn"
                  style={{padding:'12px 24px', borderRadius:'10px', background:'transparent', color:'#F9FAFB', fontWeight:500, fontSize:'14px', border:'1px solid #374151', cursor:'pointer', transition:'all .2s', fontFamily:'Inter,sans-serif'}}>
                  🔗 Copiar link
                </button>
              </div>
            </div>
          ) : (
            <div style={{background:'rgba(239,68,68,0.08)', border:'1px solid rgba(239,68,68,0.2)', borderRadius:'16px', padding:'24px', textAlign:'center'}}>
              <div style={{fontSize:'14px', color:'#EF4444', marginBottom:'8px', fontWeight:600}}>⚠️ Tu codigo de referido no esta generado aun</div>
              <div style={{fontSize:'13px', color:'#9CA3AF'}}>Cierra sesion y vuelve a iniciar para generarlo automaticamente.</div>
            </div>
          )}
          ...
          <div style={{background:'#111827', border:'1px solid rgba(255,255,255,0.06)', borderRadius:'16px', padding:'24px'}}>
            ...
            <div style={{display:'flex', flexDirection:'column', gap:'12px'}}>
              {[
                { num:'01', title:'Comparte tu codigo o link', desc:'Envia tu codigo o link a tus amigos por WhatsApp o redes sociales.' },
                { num:'02', title:'Tu amigo se registra',      desc:'Tu amigo crea su cuenta en ScoreBet e ingresa tu codigo al registrarse.' },
                { num:'03', title:'Ambos ganan puntos',        desc:'Tu recibes 300 puntos y tu amigo tambien recibe 300 puntos extra.' },
                { num:'04', title:'Sin limite de referidos',   desc:'Puedes referir a todos los amigos que quieras.' },
              ].map(s => (
                <div key={s.num} style={{display:'flex', gap:'16px', alignItems:'flex-start', background:'#0F1520', borderRadius:'12px', padding:'16px'}}>
                  <div style={{fontFamily:'JetBrains Mono,monospace', fontSize:'20px', fontWeight:700, color:'#00FF88', minWidth:'36px'}}>{s.num}</div>
                  <div>
          ...
          <div style={{background:'#111827', border:'1px solid rgba(255,255,255,0.06)', borderRadius:'16px', padding:'24px'}}>
            ...
            <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px'}}>
              {[
                { icon:'💬', label:'WhatsApp', color:'#25D366', bg:'rgba(37,211,102,0.08)', border:'rgba(37,211,102,0.2)',
                  url:`https://wa.me/?text=Unete%20a%20ScoreBet%20y%20gana%20premios%20reales%20gratis!%20Usa%20mi%20codigo:%20${codigo}%20%F0%9F%8E%81%20${encodeURIComponent(linkReferido)}` },
                { icon:'🐦', label:'Twitter/X', color:'#1DA1F2', bg:'rgba(29,161,242,0.08)', border:'rgba(29,161,242,0.2)',
                  url:`https://twitter.com/intent/tweet?text=Gana%20premios%20reales%20en%20ScoreBet%20sin%20gastar%20dinero.%20Usa%20mi%20codigo:%20${codigo}&url=${encodeURIComponent(linkReferido)}` },
                { icon:'📘', label:'Facebook', color:'#1877F2', bg:'rgba(24,119,242,0.08)', border:'rgba(24,119,242,0.2)',
                  url:`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(linkReferido)}` },
                { icon:'📋', label:'Copiar mensaje', color:'#9CA3AF', bg:'rgba(255,255,255,0.04)', border:'rgba(255,255,255,0.1)',
                  url: null },
              ].map(r => (
                <button key={r.label}
                  onClick={() => r.url
                    ? window.open(r.url, '_blank')
                    : copiar(`Unete a ScoreBet y gana premios reales gratis! Usa mi codigo: ${codigo} 👉 ${linkReferido}`)
                  }
              ...
              <table style={{width:'100%', borderCollapse:'collapse', fontSize:'13px'}}>
                <thead>
                  <tr style={{borderBottom:'1px solid rgba(255,255,255,0.06)'}}>
                    {['Referidos','Puntos ganados','Valor'].map(h => (
