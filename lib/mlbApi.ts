...
export function formatearPartidoMLB(g: any) {
  ...
  return {
    ...
    homeBadge:  (g.home_team?.abbreviation && g.home_team.abbreviation !== 'UNK') ? `https://a.espncdn.com/i/teamlogos/mlb/500/${g.home_team.abbreviation.toLowerCase()}.png` : '',
    away:       g.away_team?.abbreviation || g.away_team?.short_display_name || 'Visitante',
    awayFull:   g.away_team?.display_name || 'Visitante',
    awayBadge:  (g.away_team?.abbreviation && g.away_team.abbreviation !== 'UNK') ? `https://a.espncdn.com/i/teamlogos/mlb/500/${g.away_team.abbreviation.toLowerCase()}.png` : '',
